...
int main(int argc, char **argv) {
    ...
    // validate integer parsing and required constraints
    ...
    for (long i = 0; i < k2; i++) {
        if (i < k1) {
            A[i] = 2.0 * (double)i - 1.3;         // 0 <= i < k1
        } else {
            A[i] = (double)i / 3.0 - 2.1;         // k1 <= i < k2
        }
    }

    // S = A[0]*A[1] + A[1]*A[2] + ... + A[k2-2]*A[k2-1]
    double S = 0.0;
    for (long i = 0; i < k2 - 1; i++) {
        S += A[i] * A[i + 1];
    }

    // store final output in buffer
